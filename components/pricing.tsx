...
const plans = [
  {
    name: "Essencial",
    description: "Para artistas solo e bandas iniciantes",
    price: "79",
    period: "/mês",
    features: [
      "Agenda ilimitada",
  ...
  {
    name: "Profissional",
    description: "Para bandas e artistas em crescimento",
    price: "149",
    period: "/mês",
    features: [
      "Tudo do Essencial",
  ...
  return (
    <section id="planos" className="relative py-16 lg:py-24">
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        {/* Pricing Cards */}
        <div className="mx-auto grid max-w-4xl gap-4 sm:gap-6 lg:grid-cols-2 lg:gap-8">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative overflow-hidden rounded-xl p-6 transition-all duration-300 sm:rounded-2xl sm:p-8 ${
                plan.popular
                  ? "border border-violet-500/60 bg-gradient-to-b from-violet-500/10 to-transparent"
                  : "border border-border/40 bg-card/30 hover:border-border/60"
              }`}
            >
              {/* Popular Badge */}
              {plan.popular && (
                <div className="absolute right-3 top-3 rounded-full bg-gradient-to-r from-violet-600 to-violet-500 px-2.5 py-0.5 text-[10px] font-medium text-white sm:right-4 sm:top-4 sm:px-3 sm:py-1 sm:text-xs">
                  Mais popular
                </div>
              )}
